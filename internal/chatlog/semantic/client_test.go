...
func TestDoJSONRequestRejectsOversizedErrorBody(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusBadGateway)
		_, _ = fmt.Fprint(w, strings.Repeat("x", maxModelErrorBodyBytes+1))
	}))
	defer server.Close()

	client := &Client{httpClient: server.Client()}
	var out map[string]any
	err := client.doJSONRequest(context.Background(), "", server.URL, map[string]any{}, &out)
	if err == nil || !strings.Contains(err.Error(), "response body exceeds") {
		t.Fatalf("expected bounded error-body failure, got %v", err)
	}
}
